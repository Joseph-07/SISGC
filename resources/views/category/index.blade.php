...
@section('content')
    <div class="zoom" id="container">
        <div class="w-full bg-emerald-700 rounded-lg shadow-md text-white px-2 font-semibold text-sm">
            SISGC » Categorías » Administrar Categorías
        </div>
        <h1 class="text-3xl mb-6 text-center font-bold mt-4">Administrar Categorías</h1>
        ...
        <div class="flex mt-2">
            <a href="{{ route('categorias.create') }}" class="ml-auto mr-1 flex mt-2 zoomh bg-slate-600 hover:bg-emerald-700 text-white text-xs font-semibold p-2 rounded shadow-md ">Nuevo</a>
            <a href="{{ route('inicio') }}"
        ...
        <div class="container max-w-2xl mx-auto mt-6 ">
            ...
            <div class=" rounded-md bg-white shadow-xl">
                <div class=" bg-slate-200 w-full flex border-b-2 border-emerald-800 rounded-t-md shadow-3xl">
                    <span class="mx-auto text-md font-semibold text-gray-900 py-1">Lista de categorías</span>
                </div>
                @isset($categories)
                    ...
                    @foreach ($categories as $category)
                        <div class="bg-slate-100 border-b-2 border-emerald-700 grid grid-cols-2 py-4">
                            ...
                            <div class=" mx-auto flex">
                                <div class="zoomh">
                                    <span class="mr-1">
                                        <button class="p-2 px-0 bg-slate-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl ">
                                            ...
                                            <a href="{{ route('categorias.edit', $category) }}" class="p-2 ">
                                                Editar
                                            </a>
                                        </button>
                                    </span>
                                ...
                                <div class="zoomh">
                                    <span class="ml-1">
                                        <button id="btn-delete-{{ $category->id }}"
                                ...
                                <x-confirm id="crud-modal-{{ $category->id }}" idb="close-modal-{{ $category->id }}" 
                                    direccion="{{ route('categorias.destroy', $category) }}" />
                            </div>
                        </div>
                    @endforeach
                @endisset
