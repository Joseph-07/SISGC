    ...
    <div class="bg-black bg-opacity-50 flex h-full">
        ...
        <div class="p-4 w-full  max-w-xl mx-auto mt-24">
            ...
            <div class="relative bg-slate-200 rounded-lg shadow-3xl dark:bg-gray-100 zoom">
                ...
                @endif
                <div>
                    <form action="{{ route('terminos.update', $term->id) }}" method="POST" class="p-4">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-1">
                                @if (old('term') != null)
                                    <x-modal.input nombre="term" titulo="Termino"
                                        placeholder="Escriba el nombre del termino" tipo="text"
                                        valor="{{ old('term') }}" />
                                @else
                                    <x-modal.input nombre="term" titulo="Termino"
                                        placeholder="Escriba el nombre del termino" tipo="text"
                                        ...
                                        @foreach ($procs as $proc)
                                            @if (old('id_proc') != null)
                                                @if ($proc->id == old('id_proc'))
                                                    <option selected value="{{ $proc->id }}">{{ $proc->code }}
                                                    </option>
                                                @else
                                                    <option value="{{ $proc->id }}">{{ $proc->code }}</option>
                                                @endif
                                            @else
                                                @if ($proc->id == $term->id_proc)
                                                    <option selected value="{{ $proc->id }}">{{ $proc->code }}
                                                    </option>
                                                @else
                                                    <option value="{{ $proc->id }}">{{ $proc->code }}</option>
                            ...
                            <div class="col-span-2 mt-2">
                                ...
                                <textarea placeholder="Escriba la descripción" name="description" id="description" cols="30" rows="10"
                                    class="resize-none p-2.5 col-span-2 bg-gray-50 border border-gray-300 text-gray-900 hover:border-emerald-800 focus:ring-emerald-800  focus:border-emerald-800 text-sm rounded-lg  block w-full">@if (old('description') != null)
                                        {{ old('description') }}
                                    @else{{ $term->description }}@endif</textarea>
                            </div>
