...
@section('content')
    <div class="zoom" id="container">
        <div class="w-full bg-emerald-700 rounded-lg shadow-md text-white px-2 font-semibold text-sm">
            SISGC » Documentos » Administrar Tipos de Documentos
        </div>
        <h1 class="text-3xl mb-6 text-center font-bold mt-4">Administrar Tipos de Documentos</h1>
        ...
        <div class="flex mt-2">
            <a href="{{ route('tiposDoc.create') }}" class="ml-auto mr-1 flex mt-2 zoomh bg-slate-600 hover:bg-emerald-700 text-white text-xs font-semibold p-2 rounded shadow-md ">Nuevo</a>
            <a href="{{ route('inicio') }}"
        ...
        <div class="container max-w-2xl mx-auto mt-6 ">
            ...
            <div class=" rounded-md bg-white shadow-xl">
                <div class=" bg-slate-200 w-full flex border-b-2 border-emerald-800 rounded-t-md shadow-3xl">
                    <span class="mx-auto text-md font-semibold text-gray-900 py-1">Lista de tipos de documentos</span>
                </div>
                @isset($typeDocs)
                    ...
                    @foreach ($typeDocs as $typeDoc)
                        <div class="bg-slate-100 border-b-2 border-emerald-700 grid grid-cols-2 py-4">
                            ...
                            <div class=" mx-auto flex">
                                <div class="zoomh">
                                    <span class="mr-1">
                                        <button class="p-2 px-0 bg-slate-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl ">
                                            ...
                                            <a href="{{ route('tiposDoc.edit', $typeDoc) }}" class="p-2 ">
                                                Editar
                                            </a>
                                        </button>
                                    </span>
                                ...
                                <div class="zoomh">
                                    <span class="ml-1">
                                        <button id="btn-delete-{{ $typeDoc->id }}"
                                ...
                                <x-confirm id="crud-modal-{{ $typeDoc->id }}" idb="close-modal-{{ $typeDoc->id }}" 
                                    direccion="{{ route('tiposDoc.destroy', $typeDoc) }}" />
                            </div>
                        </div>
                    @endforeach
                @endisset
                @if (count($typeDocs) == 0)
                    <div class="bg-slate-100 border-b-2 border-emerald-700  py-4">
                        <div class="text-center">
                            <span>No hay tipos de documentos</span>
                        </div>
                    </div>
                @endif
