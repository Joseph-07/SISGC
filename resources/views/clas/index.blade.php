...
@section('content')
    <div class="zoom" id="container">
        <div class="w-full bg-emerald-700 rounded-lg shadow-md text-white px-2 font-semibold text-sm">
            SISGC » Clases » Administrar Clases
        </div>
        <h1 class="text-3xl mb-6 text-center font-bold mt-4">Administrar Clases</h1>
        ...
        <div class="flex mt-2">
            <a href="{{ route('clases.create') }}" class="ml-auto mr-1 flex mt-2 zoomh bg-slate-600 hover:bg-emerald-700 text-white text-xs font-semibold p-2 rounded shadow-md ">Nuevo</a>
            <a href="{{ route('inicio') }}"
        ...
        <div class="container max-w-2xl mx-auto mt-6 ">
            ...
            <div class=" rounded-md bg-white shadow-xl">
                <div class=" bg-slate-200 w-full flex border-b-2 border-emerald-800 rounded-t-md shadow-3xl">
                    <span class="mx-auto text-md font-semibold text-gray-900 py-1">Lista de clases</span>
                </div>
                @isset($classes)
                    ...
                    @foreach ($classes as $clas)
                        <div class="bg-slate-100 border-b-2 border-emerald-700 grid grid-cols-3 py-4">
                            ...
                            <div class=" mx-auto flex">
                                <div class="zoomh">
                                    <span class="mr-1">
                                        <button
                                            ...
                                            <a href="{{ route('clases.edit', $clas) }}" class="p-2 ">
                                                Editar
                                            </a>
                                        </button>
                                    </span>
                                ...
                                <x-confirm id="crud-modal-{{ $clas->id }}" idb="close-modal-{{ $clas->id }}"
                                    direccion="{{ route('clases.destroy', $clas) }}" />
                            </div>
                        </div>
                    @endforeach
                @endisset
